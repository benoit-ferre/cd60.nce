...
short_description: Get/Modify/Delete a single site by id
version_added: "1.0.0"
description:
  - Interact with /controller/campus/v3/sites/{id} for single-object operations.
extends_documentation_fragment:
  - cd60.nce.common
options:
  operation:
    description: Operation to perform.
    choices: [get, modify, delete]
    required: true
    type: str
  id:
...
def run_module():
    args = dict(
        operation=dict(type='str', required=True, choices=['get', 'modify', 'delete']),
    ...
    try:
        if op == 'get':
            resp = client.request('GET', f'/controller/campus/v3/sites/{sid}')
            module.exit_json(changed=False, response=resp)
        elif op == 'modify':
            resp = client.request('PUT', f'/controller/campus/v3/sites/{sid}', data=module.params.get('request'))
            module.exit_json(changed=True, response=resp)
        else:
            resp = client.request('DELETE', f'/controller/campus/v3/sites/{sid}')
            module.exit_json(changed=True, response=resp)
